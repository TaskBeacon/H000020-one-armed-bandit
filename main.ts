...
export async function run(root: HTMLElement): Promise<void> {
  ...
  await mountTaskApp({
    root,
    task_id: "H000020-one-armed-bandit",
    task_name: "One-Armed Bandit Task",
    task_description: "HTML preview aligned to local psyflow One-Armed Bandit procedure and parameters.",
    settings,
    subInfo,
    stimBank,
    buildTrials: (): CompiledTrial[] => {
      ...
      for (let blockIndex = 0; blockIndex < totalBlocks; blockIndex += 1) {
        ...
        const block = new BlockUnit({
          ...
          func: build_bandit_schedule,
          args: [blockIndex, conditionGenerationConfig]
        });

        block.conditions.forEach((condition, trialIndex) => {
          const parsedCondition = parse_bandit_condition(condition);
          const trial = new TrialBuilder({
            trial_id: next_trial_id(),
            block_id: block.block_id,
            trial_index: trialIndex,
            condition: parsedCondition.condition_id
          });
